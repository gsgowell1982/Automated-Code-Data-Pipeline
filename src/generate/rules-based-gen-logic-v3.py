...
class LLMReasoningGenerator:
    ...
    def _parse_reasoning_response(self, response: str, language: str) -> Optional[Dict]:
        ...
        for line in lines:
            ...
            if any(kw in line.lower() for kw in ['looking at', 'i see', 'observing', 'notice', '观察', '看到', '注意到']):
                if current_step:
                    steps.append(current_step)
                current_step = {"observation": line, "analysis": ""}
            elif any(kw in line.lower() for kw in ['this means', 'therefore', 'because', 'implies', '这意味着', '因此', '因为', '说明']):
                if current_step:
